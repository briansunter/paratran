...
```bash
# Basic transcription
curl http://localhost:8000/v1/audio/transcriptions -F "file=@recording.m4a"

# With beam search and sentence splitting
curl http://localhost:8000/v1/audio/transcriptions -F "file=@recording.m4a" -F "decoding=beam" -F "max_words=20"

# Extract just text
curl -s http://localhost:8000/v1/audio/transcriptions -F "file=@audio.m4a" | jq -r '.text'

# Get detailed response with segments and word timestamps
...
```json
{
  ...
  "text": "Full transcription text.",
  "segments": [
    {"id": 0, "start": 0.0, "end": 3.52, "text": "Full transcription text."}
  ],
  "words": [
    {"word": "Full", "start": 0.0, "end": 0.24},
    {"word": " transcription", "start": 0.24, "end": 0.8}
  ]
}
```
